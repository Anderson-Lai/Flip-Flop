...
const std::string& Api::CallApi(const char* url) {
	...
	if (m_result != CURLE_OK) {
		Program::Error("Failed to call api");
	}
...
Api::Api() {
	...
	if (!m_curl) {
		Program::Error("Failed to initialize curl");
	}
	else {
		// set callback fn and write data to buffer
		m_result = curl_easy_setopt(m_curl, CURLOPT_WRITEFUNCTION, WriteData);
		if (m_result != CURLE_OK) {
			Program::Error("Could not set the callback function for the curl handle");
		}

		m_result = curl_easy_setopt(m_curl, CURLOPT_WRITEDATA, &m_buffer);
		if (m_result != CURLE_OK) {
			Program::Error("Could not set the buffer for the curl handle to write the response to");
		}

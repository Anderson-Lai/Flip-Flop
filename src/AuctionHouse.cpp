...
void AuctionHouse::ScrapeAllBins() {
	...
	if (converted.is_null()) {
		Program::Error("Json parsing error occured while scraping first page of auction to update lbin data");
		return;
	}
...
void AuctionHouse::ProcessData(const json& data)
{
	...
	for (const auto& auction : data["auctions"])
	{
		...
		std::wstring name = Program::ConvertToWString(auction["item_name"]);
		std::wstring rarity = Program::ConvertToWString(auction["tier"]);
		Item::CleanName(name, auction["category"], rarity, Program::ConvertToWString(auction["item_lore"]));
...
void AuctionHouse::SeedFromFileData() {
	...
	for (auto it = fileData.begin(); it != fileData.end(); it++) {
		m_prices[Program::ConvertToWString(it.key())] = it.value().get<long long>();
	}
}
...
std::vector<Item> AuctionHouse::ScrapeNewBins(Api& getFirstPage) {
	...
	if (converted.is_null()) {
		Program::Error("Json parsing error occurred while scraping newly created bins");
		return items;
	}
	...
	for (const auto& auction : converted["auctions"]) {
		...
		items.emplace_back(
			auction["uuid"], Program::ConvertToWString(auction["item_name"]), 
			Program::ConvertToWString(auction["tier"]), Program::ConvertToWString(auction["item_lore"]),
